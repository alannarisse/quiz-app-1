...
//set initial question number to zero
let questionNumber = 0;

//set initial score to zero
let score = 0;

//Technical requirement - question/answer form + loop
function questionAnswerForm () {
    if (questionNumber < questionAnswers.length) {
        return `<form>
            <fieldset>
                <legend>
                    <h3>${questionAnswers[questionNumber].question}</h3>
                </legend>
                <label for="answerOne">
                    <input type="radio" id="answerOne" value="${questionAnswers[questionNumber].answers[0]}" name="answer" required>
                    <span>${questionAnswers[question.Number].answers[0]}</span>
                </label>
                <label for="answerTwo">
                    <input type="radio" id="answerTwo" value="${questionAnswers[questionNumber].answers[1]}" name="answer" required>
                    <span>${questionAnswers[question.Number].answers[1]}</span>
                </label>
                <label for="answerThree">
                    <input type="radio" id="answerThree" value="${questionAnswers[questionNumber].answers[2]}" name="answer" required>
                    <span>${questionAnswers[question.Number].answers[2]}</span>
                </label>
                <label for="answerFour">
                    <input type="radio" id="answerFour" value="${questionAnswers[questionNumber].answers[3]}" name="answer" required>
                    <span>${questionAnswers[question.Number].answers[3]}</span>
                </label>
                <button role="button" class="startButton">SUBMIT</button>
            </fieldset>
        </form>`;
    } else {
        renderFinalResults();
...
function correctAnswer() {
    return `<img src="${questionAnswers[questionNumber].correctImg}" alt="Woman pretends to enjoy her trifle">
        <h3>ORDER UP!</h3>
        <p>${questionAnswers[questionNumber].correctFeedback}</p>
        <button role="button" class="nextButton">NEXT QUESTION</button>`;
}

//Create HTML for wrong answers
function wrongAnswer() {
    return `<img src="${questionAnswers[questionNumber].wrongImg}" alt="Egg thrown on man's face">
        <h3>Oop, you've got egg on your face.</h3>
        <p>${questionAnswers[questionNumber].wrongFeedback}</p>
        <button role="button" class="nextButton">NEXT QUESTION</button>`;
}
